...
export class EnsureScheduleTemplatesTables1764094574480
  implements MigrationInterface
{
  name = 'EnsureScheduleTemplatesTables1764094574480';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS \`schedule_template_slots\` (
        ...
        \`slot_type\` enum('LECTURE','LAB','TUTORIAL','EXAM') NOT NULL,
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'ALTER TABLE `schedule_template_slots` DROP FOREIGN KEY `FK_schedule_template_slots_template`',
    ).catch(() => undefined);
    await queryRunner.query(
      'ALTER TABLE `schedule_templates` DROP FOREIGN KEY `FK_schedule_templates_creator`',
    ).catch(() => undefined);
    await queryRunner.query(
      'ALTER TABLE `schedule_templates` DROP FOREIGN KEY `FK_schedule_templates_department`',
    ).catch(() => undefined);
